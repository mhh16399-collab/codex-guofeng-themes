...
function Initialize-DreamSkinThemeStore {
  ...
  # Ship and refresh every reviewed Windows Guofeng preset. Existing user
  # themes are untouched because only IDs declared by the bundled catalog are
  # managed here.
  $catalogPresetSources = @{}
  $catalogPath = Join-Path $SkillRoot 'presets\catalog.json'
  if (Test-Path -LiteralPath $catalogPath -PathType Leaf) {
    $catalog = (Read-DreamSkinUtf8File -Path $catalogPath) | ConvertFrom-Json -ErrorAction Stop
    if ($catalog.schemaVersion -ne 1 -or $catalog.themes -isnot [array]) {
      throw 'Bundled Windows preset catalog is invalid.'
    }
    foreach ($catalogIdValue in @($catalog.themes)) {
      $catalogId = "$catalogIdValue"
      if ($catalogId -cnotmatch '^preset-[A-Za-z0-9_-]{1,72}$') {
        throw "Bundled Windows preset id is invalid: $catalogId"
      }
      $catalogSource = Join-Path (Join-Path $SkillRoot 'presets') $catalogId
      $catalogPack = Read-DreamSkinTheme -ThemeDirectory $catalogSource
      if ("$($catalogPack.Theme.id)" -cne $catalogId) {
        throw "Bundled Windows preset metadata id does not match its directory: $catalogId"
      }
      $catalogCss = Join-Path $catalogSource 'theme.css'
      Assert-DreamSkinSafeCssFile -Path $catalogCss
      $catalogDestination = Join-Path $paths.Saved $catalogId
      Ensure-DreamSkinManagedDirectory -Path $catalogDestination -Root $paths.Root
      $catalogImageName = [System.IO.Path]::GetFileName($catalogPack.ImagePath)
      $catalogImage = Join-Path $catalogDestination $catalogImageName
      Assert-DreamSkinNoReparseComponents -Path $catalogImage
      Copy-Item -LiteralPath $catalogPack.ImagePath -Destination $catalogImage -Force
      Assert-DreamSkinImageFile -Path $catalogImage
      Copy-Item -LiteralPath $catalogPack.ThemePath `
        -Destination (Join-Path $catalogDestination 'theme.json') -Force
      Copy-Item -LiteralPath $catalogCss `
        -Destination (Join-Path $catalogDestination 'theme.css') -Force
      $catalogPresetSources[$catalogId] = $catalogSource
    }
  }
  ...
  if (Test-Path -LiteralPath $activeTheme -PathType Leaf) {
    ...
    elseif ($catalogPresetSources.ContainsKey($activeId)) {
      $refreshSource = $catalogPresetSources[$activeId]
    }
    if ($null -ne $refreshSource) {
      ...
      $sourceCss = Join-Path $refreshSource 'theme.css'
      if (Test-Path -LiteralPath $sourceCss -PathType Leaf) {
        Assert-DreamSkinSafeCssFile -Path $sourceCss
        Copy-Item -LiteralPath $sourceCss -Destination (Join-Path $paths.Active 'theme.css') -Force
      }
    }
  }
