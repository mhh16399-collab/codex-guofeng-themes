...
6. A bundled Guofeng theme from the project gallery uses only the exact `dreamskin://preset?theme=preset-...` link. Resolve the ID from the managed saved-theme catalog, require a native confirmation, and reuse the verified transactional apply/rollback flow. Never accept a URL, filesystem path, command, extra query parameter, case-shifted scheme, or silent-apply flag.
