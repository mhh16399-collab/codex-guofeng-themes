...
- 新增汝窑天青、敦煌鎏金、青花瓷、海棠宋锦、霁夜星河五套原创主题；连同竹青、朱砂、墨韵，Windows 安装包现内置八套可一键切换的国风主题，竹青仍为全新安装默认主题。
- 新增 Codex 国风主题馆：提供八套主题的真实实机预览、明暗筛选、搜索、主题详情与 Windows 安装引导，并通过 GitHub Pages 工作流发布。
- 五张主题馆预览与五套安装器背景使用同一批已批准视觉资产，主题目录、`catalog.json` 与 Release 构建器均固定校验其完整性。

### 改进

- Windows 国风发行版本同步到 v1.7.0；Release 说明、主页、安装文档和中英文介绍同步更新为八套主题。

## 1.6.0 — 2026-08-19

### 新增

- 首发 Codex 国风主题工坊：内置竹青、朱砂、墨韵三套 Windows 主题，竹青为默认主题，并由受校验的主题目录驱动一键切换。
...
- 同步 v1.6.0 版本号，启用 Windows-only 国风首发工作流与品牌化安装包名称。
