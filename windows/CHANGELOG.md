...
- 完成内置主题的第二轮界面适配，统一处理顶部渐隐、输入区边界、图片预览操作和任务页可读性。
- 金陵云锦侧栏改为更轻透的孔雀蓝覆盖，并移除模糊，保留织锦背景层次。

## 1.7.2 — 2026-08-22

- 千里江山首页继续保留完整山水画幅，任务详情页改为弱化背景与 90% 宣纸阅读层，避免长文直接压在山体上。
- 将 Codex 原生粉紫色表面、控件、强调色和主按钮映射到当前皮肤色板，并移除首页输入框上重复且被裁切的英文口号。
- 开机启动改用当前用户的 `Run` 注册表项，不再向 Startup 文件夹写入快捷方式，修复安装时报 `0x80070005` 拒绝访问。

> 此处版本号属于 Codex Guofeng Themes 安装包，不是 Codex 客户端版本。

## 1.7.1 — 2026-08-22

- 修复托盘把已经退出的注入器误报为“运行中”的状态错误。
- Windows 安装包内置并升级刷新十八套国风主题，包含完整版“千里江山”样式。
- 升级时同步刷新当前正在使用的官方主题 CSS，无需先卸载旧版。
- 国风主题馆的十八套内置主题使用严格的 `dreamskin://preset?theme=preset-...` 一键换肤协议。Windows 客户端只从受管理的本地预设目录解析精确 ID，显示原生确认，并复用可见验证、失败回滚与恢复机制；协议不接受 URL、文件路径、命令或额外参数。
- 主题馆卡片改为圆角外框与内缩预览环，新增客户端唤起和完整大图详情操作；主图、卡片和详情继续保持原图比例，不裁切主题预览。
- 新增千里江山、景泰华蓝、黑漆螺钿、茶烟松风、榫卯丹楹、瑞鹤凌霄、唐三彩、汉简墨痕、洛水流霞、金陵云锦十套原创主题；主题馆、Windows 预设目录、修复安装清单和发行哈希锁定表同步扩展到十八套。
- 金陵云锦采用孔雀蓝、宝石绿与真金线的孔雀羽团花方向，避免与暖红花卉取向的海棠宋锦重复。
