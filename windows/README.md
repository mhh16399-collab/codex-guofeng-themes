...
Codex Guofeng Themes 通过本机回环 CDP 给官方 Codex Windows 桌面应用加载外部主题。当前安装包内置竹青、朱砂、墨韵、汝窑天青、敦煌鎏金、青花瓷、海棠宋锦、霁夜星河、千里江山、景泰华蓝、黑漆螺钿、茶烟松风、榫卯丹楹、瑞鹤凌霄、唐三彩、汉简墨痕、洛水流霞、金陵云锦，并会继续增加原创国风主题。所有主题都保留原生侧栏、项目选择、任务内容和输入框，不修改 WindowsApps、`app.asar` 或应用签名。
...
Codex 国风主题馆中的内置主题使用 `dreamskin://preset?theme=preset-...`。客户端只会在
`%LOCALAPPDATA%\CodexDreamSkin` 的受管理主题目录中查找完全匹配的内置预设，显示原生确认后复用同一套
切换、可见渲染验证与失败回滚流程。该链接同样不能携带下载地址、文件路径、命令或额外参数；使用网页按钮前
需要先安装包含该协议处理器的最新版 Windows 客户端。
