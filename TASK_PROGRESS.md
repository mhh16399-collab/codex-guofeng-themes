...
## 兰亭曲水、燕山秋色、赤壁夜航三套主题（2026-08-30）

- [目标] 将用户确认的三张 Windows Codex 预览落成三套可导入主题：兰亭曲水、燕山秋色、赤壁夜航。
- [视觉约束] 三套共用现有 Windows Codex 布局；左侧是独立清晰画面，不使用背景虚化或毛玻璃；顶栏横向统一；不得把任何 Codex UI、文字、输入框或图标烘焙进背景图。
- [范围] 新增三套 `theme.json`、Safe CSS、无 UI 纯背景和严格三文件 ZIP；先不发布网站、不提升客户端版本、不覆盖当前安装运行时。
- [完成] 三套主题均已落为严格三文件主题包（`theme.json`、`theme.css`、`background.png`），并导入 `%LOCALAPPDATA%\CodexDreamSkin\themes`；导入过程不切换活动主题。
- [预览] 隔离 Windows 预览使用固定合成会话，三套均完成 1680×936 对照 QA；左右界面结构一致，左栏独立构图，顶栏无杂色，CSS 不含 blur。
- [验证] 主题合同 2/2、网站契约 17/17、运行时专项 6/6、ZIP 导入安全测试和完整 Windows 回归均退出 0；Vite production build 成功。
- [限制] 当前 Codex 会话未以受验证 CDP/Dream Skin 会话启动，无法在不中断本对话的前提下做真实窗口热注入；未重启或终止用户当前 Codex。未发布网站、未提升客户端版本、未覆盖安装运行时。
- [发布中] 用户于 2026-08-31 明确要求发布。远端 `origin/main@372467d` 已有 23 套馆藏；本次将兰亭曲水、燕山秋色、赤壁夜航加入画廊、严格 ZIP 下载和合成预览，目标馆藏 26 套。生产页面已在本地浏览器核对 `馆藏 26 / 26`、第 4/5 页卡片、三套协议链接与下载链接，控制台无错误；等待完整回归、范围提交、推送和 Pages 实际地址验证。
